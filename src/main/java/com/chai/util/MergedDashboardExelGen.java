...
public class MergedDashboardExelGen extends AbstractExcelView {
	...
	protected void buildExcelDocument(Map<String, Object> model, 
			...
			HttpServletResponse response) throws Exception {
                        String levelName = "";
		try {
			List<LabelValueBean> mergedHeaderList=(List<LabelValueBean>) model.get("mergedlistwithfacilitylevel");
			JSONArray data = (JSONArray) model.get("export_data");
			System.out.println("exel data in json" + data);
			System.out.println("level Name in json" + mergedHeaderList.get(0).getValue());
			HSSFSheet worksheet = workbook.createSheet("Dashboard Report");
			HSSFRow rowHeader=worksheet.createRow(0);
			int i=0;
                        if(levelName.equals("")){
                            levelName = mergedHeaderList.get(0).getValue();
//                            mergedHeaderList.remove(0);// for remove lga from product name field
                        }
			for (LabelValueBean mergedHeader : mergedHeaderList) {
				HSSFCell cell=rowHeader.createCell(i);
				cell.setCellValue(mergedHeader.getLabel());
...
//                                System.out.println("data to be in excel row" + data.toString());
				...
				for (LabelValueBean mergedHeader : mergedHeaderList) {
			    	...
			    	while(keys.hasNext()){
			    		...
			    		if(key.contains(mergedHeader.getLabel())){
			    			HSSFCell cell1=row.createCell(j);
			    			JSONObject obj=(JSONObject) rowObject.get(key);
//                                                System.out.println("Key " + key);
                                                cell1.setCellValue((obj.getString("FACIITY_STATUS")));

...
public class CapacityDashboardExelGen extends AbstractExcelView {
	...
	protected void buildExcelDocument(Map<String, Object> model, 
			...
			HttpServletResponse response) throws Exception {
                        String levelName = "";
		try {
			List<LabelValueBean> capacityHeaderList=(List<LabelValueBean>) model.get("capacitylistwithfacilitylevel");
			JSONArray data = (JSONArray) model.get("export_data");
			System.out.println("exel data in json" + data);
			System.out.println("level Name in json" + capacityHeaderList.get(0).getValue());
			HSSFSheet worksheet = workbook.createSheet("Dashboard Report");
			HSSFRow rowHeader=worksheet.createRow(0);
			int i=0;
                        if(levelName.equals("")){
                            levelName = capacityHeaderList.get(0).getValue();
//                            capacityHeaderList.remove(0);// for remove lga from product name field
                        }
			for (LabelValueBean capacityHeader : capacityHeaderList) {
				HSSFCell cell=rowHeader.createCell(i);
				cell.setCellValue(capacityHeader.getLabel());
...
//                                System.out.println("data to be in excel row" + data.toString());
				...
				for (LabelValueBean capacityHeader : capacityHeaderList) {
			    	...
			    	while(keys.hasNext()){
			    		...
			    		if(key.contains(capacityHeader.getLabel())){
			    			HSSFCell cell1=row.createCell(j);
			    			JSONObject obj=(JSONObject) rowObject.get(key);
//                                                System.out.println("Key " + key);
			    			cell1.setCellValue(obj.getString("FACIITY_CAPACITY"));

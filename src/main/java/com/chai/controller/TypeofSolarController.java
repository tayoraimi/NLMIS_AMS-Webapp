 ...
 */
@Controller
public class TypeofSolarController {
    ...
    @RequestMapping(value = "/typeofsolar", method = RequestMethod.GET)
    public ModelAndView showFunctionChart(HttpServletRequest request, HttpServletResponse respones) {
        ModelAndView modelAndView = new ModelAndView("TypeOfSolar");
        try {
    ...
    @RequestMapping(value = "/get_type_of_solar_data")
    public JSONArray getTypeOfSolarData(HttpServletRequest request, HttpServletResponse respones, @RequestParam("filterLevel") String filterLevel) {
...
//		System.out.println("week :" + week);
        try {
            ...
            data = new AMSChartServices().getTypeOfSolarData(userBean, sclevel);

            System.out.println("get_type_of_solar_data jsonforjsp ======" + data.toString());

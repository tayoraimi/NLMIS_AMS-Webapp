 ...
 */
@Controller
public class WardsWithOtherCCEController {
    ...
    @RequestMapping(value = "/wardswithothercce", method = RequestMethod.GET)
    public ModelAndView showFunctionChart(HttpServletRequest request, HttpServletResponse respones) {
		ModelAndView modelAndView= new ModelAndView("WardsWithOtherCCE");
		try {
    ...
    @RequestMapping(value = "/get_ward_with_other_cce_data")
    public JSONArray getWardWithOtherCCEData(HttpServletRequest request, HttpServletResponse respones, @RequestParam("filterLevel") String filterLevel) {
...
//		System.out.println("week :" + week);
        try {
            ...
            data = new AMSChartServices().getWardWithOtherCCEData(userBean, sclevel);

            System.out.println("get_ward_with_other_cce_data jsonforjsp ======" + data.toString());

 ...
 */
@Controller
public class WardsWithSolarController {
    ...
    @RequestMapping(value = "/wardswithsolar", method = RequestMethod.GET)
    public ModelAndView showFunctionChart(HttpServletRequest request, HttpServletResponse respones) {
		ModelAndView modelAndView= new ModelAndView("WardsWithSolar");
		try {
    ...
    @RequestMapping(value = "/get_ward_with_solar_data")
    public JSONArray getWardWithSolarData(HttpServletRequest request, HttpServletResponse respones, @RequestParam("filterLevel") String filterLevel) {
...
//		System.out.println("week :" + week);
        try {
            ...
            data = new AMSChartServices().getWardWithSolarData(userBean, sclevel);

            System.out.println("get_ward_with_solar_data jsonforjsp ======" + data.toString());
